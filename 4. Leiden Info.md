...
in **4. OptimumLeidenCluster.Info.csv**, I will be referring to olny these .csv files from hereon out in my R code.
...
An optimum Leiden cluster is where each community contains patients with the same diagnosis, with each community containing a sizeable number of nodes (indicating that the "diagnosis" of these nodes may be correlated to the fact that they were all clustered into the same community). We define 2 metrics which helps us determine the optimum Leiden clustering parameters.
...
![image](https://github.com/user-attachments/assets/9a125ee1-dfea-47c4-a68b-63cc375e0164)
...
Which has a Weighted Purity of **0.7734** and a Patient Capture (%) of **70.72**.
